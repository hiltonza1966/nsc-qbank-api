...
def extract_caps_topics(pdf_path):
    ...
    text = re.sub(r'\.{3,}', '', text)  # Remove dot sequences
    text = re.sub(r'\s+', ' ', text)
    
    # Find Section 2.4 - Focus of Content Areas
    topics = []
    section2_4_match = re.search(r'2\.4.*?Focus of Content areas?(.*?)(?:2\.5|3\.|SECTION 3|seCtion 3)', text, re.IGNORECASE | re.DOTALL)
    ...
    term_topics = []
    section3_2_3_match = re.search(r'3\.2\.3.*?Topic allocation per term(.*?)(?:4\.|SECTION 4|seCtion 4|Assessment)', text, re.IGNORECASE | re.DOTALL)
    ...
    if section3_2_3_match:
        ...
        for grade, term, content in grade_matches:
            topic_parts = [p.strip() for p in re.split(r'[,\n]', content) if p.strip() and p.strip().lower() not in ['revision', 'exams', 'assessment']]
