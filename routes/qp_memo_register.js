...
async function getDatabaseData(req, res) {
  const connection = await db.getConnection();
  try {
    // Get all unique paper codes from item_master
    const [paperCodes] = await connection.execute(
      'SELECT DISTINCT source_paper_code FROM item_master ORDER BY source_paper_code'
    );
    ...
    const allPapers = [];
    for (const paperCode of allPaperCodes) {
      // Get item counts and marks from item_master
      const [itemCounts] = await connection.execute(
        'SELECT COUNT(*) as count, CAST(SUM(marks) AS UNSIGNED) as total_marks, CAST(SUM(qp_marks) AS UNSIGNED) as total_qp_marks, CAST(SUM(memo_marks) AS UNSIGNED) as total_memo_marks FROM item_master WHERE source_paper_code = ?',
        [paperCode]
      );
      ...
      const [memoCounts] = await connection.execute(
        'SELECT COUNT(*) as count, CAST(SUM(im.marks) AS UNSIGNED) as total_memo_marks FROM item_memos im JOIN item_master m ON im.item_id = m.item_id WHERE m.source_paper_code = ?',
        [paperCode]
      );
      // Get sub-part count
      ...
      // Get duplicate count
      const [dupCount] = await connection.execute(
        'SELECT COUNT(*) as count FROM (SELECT question_number FROM item_master WHERE source_paper_code = ? GROUP BY question_number HAVING COUNT(*) > 1) as dups',
        [paperCode]
      );

      // Get subject info from first item
      const [firstItem] = await connection.execute(
        'SELECT subject_official_code, subject_alpha_code, paper_no, year_id, grade_id FROM item_master WHERE source_paper_code = ? LIMIT 1',
        [paperCode]
      );
      ...
      let gradeValue = '';
      try {
        const [subj] = await connection.execute(
          'SELECT subject_name FROM lookup_subjects WHERE subject_alpha_code = ? LIMIT 1',
          [item.subject_alpha_code || '']
        );
        if (subj.length > 0) subjectName = subj[0].subject_name;
        const [yr] = await connection.execute(
          'SELECT year_value FROM lookup_years WHERE year_id = ? LIMIT 1',
          [item.year_id || 0]
        );
        if (yr.length > 0) yearValue = yr[0].year_value;
        const [gr] = await connection.execute(
          'SELECT grade_value FROM lookup_grades WHERE grade_id = ? LIMIT 1',
          [item.grade_id || 0]
        );
      ...
      const paper = {
        paper_code: paperCode,
        display_paper_code: paperCode,
        subject_code: parsed?.subject_code || item.subject_alpha_code || '',
        subject_name: subjectName || parsed?.subject_code || '',
        subject_alpha_code: item.subject_alpha_code || parsed?.subject_code || '',
        subject_official_code: item.subject_official_code || '',
        paper_no: item.paper_no || parsed?.paper_no || '',
        year: yearValue || parsed?.year || '',
        session: parsed?.session || '',
        language: parsed?.language || '',
        expected_pdf_marks: totalQpMarks || 0,
        ...
        memo_expected_marks: totalMemoMarks || 0,
        marks_match: totalQpMarks === totalMemoMarks,
        marks_variance: (totalQpMarks || 0) - (totalMemoMarks || 0),
        qp_corrected_marks: totalMarks || 0,
        memo_corrected_marks: totalMemoMarks || 0,
        corrected_marks_match: totalMarks === totalMemoMarks,
        corrected_marks_variance: (totalMarks || 0) - (totalMemoMarks || 0),
        has_errors: false,
        error_count: 0,
        data_quality_issues: [],
        duplicate_count: dupCount[0]?.count || 0,
        pdf_marks_available: true
      };
      allPapers.push(paper);
    }
    ...
    const assessmentBodies = [...new Map(allPapers.map(p => [p.assessment_body_id, {
      assessment_body_id: p.assessment_body_id,
      body_name: 'Default Body'
    }])).values()];

    const assessmentTypes = [...new Map(allPapers.map(p => [p.assessment_type_id, {
      assessment_type_id: p.assessment_type_id,
    ...
    const summary = {
      ...
      total_qp_items: allPapers.reduce((sum, p) => sum + p.qp_item_count, 0),
      total_memo_items: allPapers.reduce((sum, p) => sum + p.memo_item_count, 0),
      total_expected_marks: allPapers.reduce((sum, p) => sum + (parseInt(p.qp_expected_marks) || 0), 0),
      total_pdf_marks: allPapers.reduce((sum, p) => sum + (parseInt(p.expected_pdf_marks) || 0), 0),
      total_qp_marks: allPapers.reduce((sum, p) => sum + (parseInt(p.qp_corrected_marks) || 0), 0),
      total_memo_marks: allPapers.reduce((sum, p) => sum + (parseInt(p.memo_corrected_marks) || 0), 0),
      matched_items: allPapers.filter(p => p.items_match).length,
      matched_marks: allPapers.filter(p => p.marks_match).length,
      matched_corrected_marks: allPapers.filter(p => p.corrected_marks_match).length,
      records_with_errors: allPapers.filter(p => p.has_errors).length,
    ...
    res.json({
      ...
      filters: {
        ...
        sessions: sessions.map(s => ({ session_code: s, session_name: s })),
        grades: grades.map(g => ({ grade_id: g, grade_name: `Grade ${g}` })),
        languages: languages.map(l => ({ language_code: l, language_name: l })),
        years: years.map(y => ({ year_id: y, year_value: y }))
      },
      summary,
      diagnostics
    });
