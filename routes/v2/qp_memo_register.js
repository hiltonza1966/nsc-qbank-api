  ...
  try {
    ...
    for (const paperCode of allPaperCodes) {
      ...
      const [itemCounts] = await connection.execute(
        'SELECT COUNT(*) as count, SUM(marks) as total_marks, SUM(qp_marks) as total_qp_marks, SUM(memo_marks) as total_memo_marks FROM item_master WHERE source_paper_code = ?',
        [paperCode]
      );
