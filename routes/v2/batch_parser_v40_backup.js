...
async function autoPromoteSession(db, sessionId, paperCode, dimensions, parseResult, outputDir) {
  ...
  try {
    // Get subject info from lookup_subjects
    const [subjectRows] = await connection.execute(
      'SELECT subject_id, subject_official_code, subject_alpha_code FROM lookup_subjects WHERE subject_alpha_code = ? LIMIT 1',
      [dimensions.subject_code || paperCode.split('_')[0]]
    );
    const subject = subjectRows[0] || {};

    // Get year_id from lookup_years
    const [yearRows] = await connection.execute(
      'SELECT year_id FROM lookup_years WHERE year_value = ? LIMIT 1',
      [dimensions.year || paperCode.split('_')[2]]
    );
    const yearId = yearRows[0]?.year_id || null;

    // Get grade_id from lookup_grades (Grade 12)
    const [gradeRows] = await connection.execute(
      'SELECT grade_id FROM lookup_grades WHERE grade_value = ? LIMIT 1',
      [12]
    );
    const gradeId = gradeRows[0]?.grade_id || null;

    // Get paper_id from lookup_papers
    const [paperRows] = await connection.execute(
      'SELECT paper_id FROM lookup_papers WHERE paper_no = ? LIMIT 1',
      [dimensions.paper_no || paperCode.split('_')[1]]
    );
    const paperId = paperRows[0]?.paper_id || null;

    const assessmentTypeId = dimensions.assessment_type_id || 1;
    const assessmentBodyId = dimensions.assessment_body_id || 1;

    // Get all parsed items for this paper
    const [parseItems] = await connection.execute(
      'SELECT result_id, question_number, question_text, expected_marks, auto_corrected_marks, correction_status, parser_extracted_marks, is_header, parent_header_id FROM parse_results WHERE paper_code = ? AND is_memo = 0',
      [paperCode]
    );
    ...
    for (const item of parseItems) {
      ...
      const marks = item.auto_corrected_marks || item.expected_marks || 0;

      // EXPLICIT: 27 columns = 27 placeholders = 27 values
      await connection.execute(
        `INSERT INTO item_master (
          item_id, item_hash, item_code,
          subject_id, subject_official_code, subject_alpha_code,
          paper_no, year_id, grade_id, paper_id,
          assessment_type_id, assessment_body_id,
          question_number, question_text,
          marks, marks_allocated, qp_marks, memo_marks,
          source_paper_code, source_question_number, source_year,
          status, review_status, parser_confidence,
        ...
        [
          ...
          'active',
          'draft',
          item.correction_status === 'auto_corrected' ? 1 : 0,
          item.is_header || 0,
          item.parent_header_id || null,
          1
        ]
      );
    ...
    console.log('Auto-promoted ' + paperCode + ': ' + promotedCount + ' items, ' + skippedCount + ' skipped');
    return { success: true, promoted: promotedCount, skipped: skippedCount };
  } catch (error) {
    console.error('Auto-promote error for', paperCode, error.message);
    throw error;
  } finally {
    connection.release();
  }
