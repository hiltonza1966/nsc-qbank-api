...
async function check() {
  ...
  const [cols] = await c.execute(`SELECT COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, IS_NULLABLE, COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'item_memos' ORDER BY ORDINAL_POSITION`);
  console.log('item_memos columns (' + cols.length + '):');
  cols.forEach((col, i) => {
