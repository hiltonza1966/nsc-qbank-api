# QPMemoRegister Embedded Attachment Fix
# Run from C:
sc-qbank in PowerShell

$repo = "C:\dev\nsc-qbank"
$registerFile = "$repo\frontend\src\pages\QPMemoRegister.tsx"
$scriptPath = "$repo\replace_attachments_embed.js"

# Backup the current Register
copy "$registerFile" "$registerFile.bak.pre-embed-fix"
...
// FIX 1: Add QP Image Gallery inline in the Question Item panel
const questionTextLabel = "<div style={{ fontSize: '12px', fontWeight: '600', color: '#374151', marginBottom: '4px' }}>Question Text</div>";
const qtIdx = content.indexOf(questionTextLabel);
if (qtIdx === -1) { console.error('ERROR: Question Text label not found'); process.exit(1); }
...
// FIX 2: Replace the Attachments Section
const startMarker = '                  {/* Attachments Section */}';
const startIdx = contentWithGallery.indexOf(startMarker);
if (startIdx === -1) { console.error('ERROR: Attachments Section not found'); process.exit(1); }
...
if (loadingIdx === -1) { console.error('ERROR: Loading marker not found'); process.exit(1); }

const endDivIdx = contentWithGallery.indexOf('</div>', loadingIdx + loadingMarker.length);
if (endDivIdx === -1) { console.error('ERROR: Closing </div> not found'); process.exit(1); }
...
Set-Content -Path $scriptPath -Value $jsCode -Encoding UTF8
Write-Host "Script written to $scriptPath"
Write-Host ""
Write-Host "Running replacement..."
node $scriptPath $registerFile
