# QBank Development Plan – Corporate Edition v9
**Date:** 12 June 2026
**Updated:** Document Synchronization — v13 Status, Natural Keys Migration FAILED, Frontend White Screen Unresolved
**Status:** Phase 1 Partially Complete, Phase 2 Schema Applied but NOT Verified, Phase 3-6 Not Started
**Architecture:** Enterprise-grade Question Bank System with 39 Tables (34 + 5 Security)
...
**Natural Keys Migration:** ATTEMPTED but FAILED
...
## COMPLETE DATABASE SCHEMA (39 Tables) — STATUS PER TABLE
...
|-------|---------|--------|
| `item_master` | Core item table (all 6 dimensions + content + classification + security fields v12) | Created, 0 rows |
...
| `parse_results` | Parser output with auto-correction + RED flags | Created, 0 rows (comparison engine NOT verified) |
...
### Category 9: Security Tables (5 tables) — ALL CREATED, ALL EMPTY (Added v12)
| Table | Purpose | Status |
|-------|---------|--------|
| `tool_audit_log` | Every tool action logged | Created, 0 rows |
| `secure_media_storage` | Encrypted media references | Created, 0 rows |
| `sandbox_config` | CSP policies per tool | Created, 0 rows |
| `subject_tool_mapping` | Auto-detects required tools | Created, 0 rows |
| `workflow_rules` | Transition rules for workflows | Created, 0 rows |

**Total: 39 tables (15 lookup + 19 transactional + 5 security)**
...
- [x] Auto-correction fixes mark discrepancies within tolerance (<=2x expected)
- [x] RED flags for manual review when parser unreliable or variance > 2x
...
|----------|--------|--------|
| Complete 39-table schema | OK | All tables created + seeded |
| CAPS curriculum populated | PARTIAL | Life Sciences Grade 12 topics done, subtopics NOT done |
| Parse all sections (A, B, C) from QP | OK | 100% extraction (items only, no marks) |
| Extract and store images/diagrams | FAIL | Not started |
| Manual editing of parsed items | PARTIAL | UploadWizard + ReviewPanel created, NOT verified |
| Review workflow (3 levels) | FAIL | Not started |
| Paper assembly with constraints | FAIL | Not started |
| Examiner flexibility (replace, shuffle) | FAIL | Not started |
| Tag taxonomy (controlled vocabulary) | OK | Admin + SME |
| Item versioning | OK | Full audit trail (schema ready) |
| Exposure tracking | OK | Usage statistics (schema ready) |
| React frontend | PARTIAL | Modern UI (UploadWizard + ReviewPanel done, rest NOT started, white screen unresolved) |
| Export to PDF/Word | FAIL | Not started |
| Analytics dashboard | FAIL | Not started |
| CAPS PDF Parser | FAIL | v2.7a deployed but broken |
| Natural Keys Migration | FAIL | ATTEMPTED but FAILED |
| Security Tables (v12) | OK | Created, empty |
...
| Natural keys migration failure | High | High | **DO NOT retry without complete redesign and backup** |
...
### Commit 8: Security Tables (v12) — COMPLETE
- `git commit -m "security: add v12 security tables, audit logging, sandbox config, tool mapping"`
- **Hash:** `8bb34a4` — Latest commit as of 2026-06-12
...
13. **Security-first:** All tools built-in, sandboxed, audit-logged, encrypted media storage
14. **Natural keys NOT implemented:** System uses surrogate keys. Natural key columns exist as nullable fields only.

---

## PHASE 7: Natural Keys Implementation (Option 2) — ATTEMPTED BUT FAILED
...
### 7.2 Solution: Natural Keys Throughout — ATTEMPTED BUT FAILED
...
### 7.3 Implementation Steps — ATTEMPTED BUT FAILED
1. Update lookup_subjects: Drop subject_id, set subject_official_code as PK — **FAILED**
2. Update lookup_papers: Drop paper_id, set paper_no as PK — **FAILED**
3. Update lookup_assessment_bodies: Drop assessment_body_id, set assessment_origin as PK — **FAILED**
4. Update ALL transactional tables to use natural keys instead of surrogate IDs — **FAILED**
5. Update backend routes to use natural keys — **FAILED**
6. Update frontend to use natural keys — **FAILED**

### 7.4 Failure Details
- **File:** natural_keys_final_v2.zip
- **Error:** "Table 'nsc_qbank.papers' doesn't exist"
- **Root cause:** Migration referenced incorrect table names or assumed tables that don't exist
- **Impact:** System still uses surrogate keys. No data loss reported.

### 7.5 Status: FAILED — DO NOT RETRY WITHOUT EXPLICIT USER APPROVAL
...
- **Decision pending:** User has NOT approved re-attempt
- **Next AI must:** Check `SHOW TABLES` and `SHOW COLUMNS` before any schema change

---

*End of Development Plan – Corporate Edition v9*
*Schema complete: 39 tables created, CAPS parser v2.7a broken, Comparison engine fix applied but not verified, Natural keys migration FAILED*
*Date: 2026-06-12 18:57*
