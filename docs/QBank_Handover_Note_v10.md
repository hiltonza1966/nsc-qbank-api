...
### 2.0 Schema Migration Status (UPDATED: 2026-06-09 18:00)
**Migration 014 Complete:**
- ✅ All 34 new tables created successfully
- ✅ 66 foreign key constraints established
- ✅ 15 lookup tables seeded with data
- ✅ 123 subjects synced from nsc_registration_v3.lookup_subjects
- ✅ 38 items populated in parse_expected_structure for LIFE_SC_P1_NOV_2025
- ✅ Backend running on port 4000 with all API endpoints active
- ✅ Old tables dropped (qbank_items, qbank_papers, etc.)
- ✅ Legacy tables preserved (accounting_questions, qbank_users_legacy)

**Key Changes Made:**
- Table names updated: QB_questionP_Structure → parse_expected_structure, QB_parse_sessions → parse_sessions, QB_parsed_results → parse_results
- lookup_subjects uses subject_official_code (not subject_code) to match source table
- paper_code column added to parse_expected_structure for wizard compatibility
- Stored procedure sync_lookup_subjects() created for manual sync by Superadmin
...
### 2.4 Natural Keys Implementation (OPTION 2 - NEXT PHASE)
**Decision:** Change from surrogate keys (INT auto-increment) to natural keys (business codes) throughout all tables.

**Current Problem:**
- Schema uses surrogate keys: subject_id (INT), paper_id (INT), assessment_body_id (INT)
...
**Option 2 Solution:**
- Replace subject_id with subject_official_code (VARCHAR) as primary key in lookup_subjects
- Replace paper_id with paper_no (INT) as primary key in lookup_papers
- Replace assessment_body_id with assessment_origin (VARCHAR) as primary key in lookup_assessment_bodies
- Update ALL foreign key references across all 34 tables to use natural keys
- Aligns directly with nsc_registration_v3.subject_structure
...
- ✅ Direct use of DBE official codes throughout all tables
- ✅ Simpler queries: no joins to lookup tables for basic identification
- ✅ Immutable: DBE subject codes rarely change

**Implementation Plan:**
1. Update lookup_subjects: subject_official_code (VARCHAR) as PK instead of subject_id (INT)
2. Update lookup_papers: paper_no (INT) as PK instead of paper_id (INT)
3. Update lookup_assessment_bodies: assessment_origin (VARCHAR) as PK instead of assessment_body_id (INT)
4. Update ALL transactional tables (item_master, parse_expected_structure, etc.) to use natural keys
5. Update ALL foreign key constraints
6. Update ALL backend routes to use natural keys in queries
7. Update frontend API calls to use natural keys
8. Test end-to-end with wizard upload

**Tables Affected:**
- lookup_subjects, lookup_papers, lookup_assessment_bodies (PK changes)
- item_master, item_stimuli, parse_sessions, parse_expected_structure (FK changes)
- paper_templates, generated_papers (FK changes)
- All other tables referencing these dimensions

**Commit Message:** `feat: implement natural keys (Option 2) - subject_official_code, paper_no, assessment_origin`
