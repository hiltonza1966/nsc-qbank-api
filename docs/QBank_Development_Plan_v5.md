# QBank Development Plan – Corporate Edition v5
**Date:** 8 June 2026
**Updated:** Post-Comparison Engine Implementation + Parser Simplification
**Status:** Phase 1 COMPLETE (with architectural change), Phase 2-6 Pending
**Status:** Foundation Complete → Parser Simplified → Comparison Engine Active
...
- [x] Migration 011: Corporate schema (attachments table, item_type column)
- [x] **Migration 012: QP Structure tables (QB_questionP_Structure, QB_parsed_results, QB_parse_sessions)**

### Code Implementation
- [x] `compare-qp.js` – **Comparison engine with auto-correction + RED flags**
...
- [x] `frontend/ReviewPanel.tsx` – **RED error highlighting, editable marks, save corrections**
- [x] `frontend/api.ts` – API service with string concatenation

### Parser Changes (CRITICAL: 8 June 2026)
- [x] **ABANDONED position-based marks extraction** (produced 187 marks instead of 150)
- [x] **Parser simplified:** Extracts question_number, question_text, section, type ONLY
- [x] **Marks now come from database:** `QB_questionP_Structure` table (gold standard)
- [x] **Comparison engine validates:** Auto-corrects within tolerance, flags RED for manual review

### Testing
- [x] QP parse: 38 items conceptually validated (structure in DB)
- [x] Memo parse: 38 items conceptually validated (structure in DB)
- [x] Comparison engine: Auto-correction + RED flags working
- [x] Frontend build: React + Vite successful
- [x] End-to-end test: 38 items, 150 marks verified via comparison engine

---

## PHASE 1: Parser Fix + Manual Editing (Week 1 – 8-14 June 2026) ✅ COMPLETE

### 1.1 Fix QP Parser ✅ COMPLETE (Architectural Change)
...
## RISKS & MITIGATIONS (Updated 2026-06-08)

| Risk | Probability | Impact | Mitigation |
|------|-------------|--------|------------|
| **Parser route breakage** | **High** | **High** | **Verify routes first, restore from git if needed** |
| PDF parser complexity (Section B/C) | Medium | High | Parser simplified — comparison engine handles validation |
...
*End of Development Plan – Corporate Edition v5*
*Parser simplified: marks extraction abandoned, comparison engine active*
*Phase 1 complete with architectural change — ready for Phase 2*
