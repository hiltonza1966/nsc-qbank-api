#!/usr/bin/env python3
"""
QBank CAPS Cleanup & Reseed Script
1. Clears existing CAPS data
2. Reseeds from CAPS_Full_ATP_Master.xlsx
"""
...
def generate_cleanup_and_seed_sql():
    ...
    sql_lines = []
    sql_lines.append("-- ============================================")
    sql_lines.append("-- CAPS Topics & Subtopics Cleanup + Reseed")
    ...
    sql_lines.append("DELETE FROM lookup_caps_subtopics;")
    sql_lines.append("DELETE FROM lookup_caps_topics;")
    sql_lines.append("")
    sql_lines.append("-- Reset auto-increment")
    sql_lines.append("ALTER TABLE lookup_caps_subtopics AUTO_INCREMENT = 1;")
    sql_lines.append("ALTER TABLE lookup_caps_topics AUTO_INCREMENT = 1;")
    sql_lines.append("")
    ...
    sql_file = "caps_atp_cleanup_reseed.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Cleanup + Reseed SQL saved to: {sql_file}")
