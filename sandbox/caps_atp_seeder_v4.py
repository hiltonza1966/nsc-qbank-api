#!/usr/bin/env python3
"""
QBank CAPS Database Seeder - FIXED v4
Shortens topic_code to fit varchar(20) constraint.
"""
...
def generate_seed_sql():
    ...
    sql_lines.append("-- FIXED v4: shortened topic_code for varchar(20)")
    sql_lines.append("-- ============================================")
    sql_lines.append("")
    ...
    for subject, topics in sorted(subject_topics.items()):
        ...
        for topic_name, topic_data in sorted(topics.items()):
            ...
            # FIXED: Generate short unique topic_code (max 20 chars)
            # Format: SUB_ + topic abbreviation (max 16 chars total)
            topic_abbr = re.sub(r'[^a-zA-Z0-9]', '', topic_name[:12]).upper()
            topic_code = f"{subject_prefix}_{topic_abbr}"
            # Ensure max 20 chars
            if len(topic_code) > 20:
                topic_code = topic_code[:20]
            ...
            sql_lines.append(f"    {strand},")
            sql_lines.append(f"    {term},")
            sql_lines.append(f"    {topic_code},")  # FIXED: max 20 chars
    ...
    sql_file = "caps_atp_seed_v4.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Fixed SQL v4 saved to: {sql_file}")
