#!/usr/bin/env python3
"""
QBank CAPS Database Seeder - FIXED SQL Syntax
Uses proper INSERT syntax with VALUES and subquery for subject_official_code.
"""
...
def generate_seed_sql():
    ...
    sql_lines.append("-- FIXED SQL Syntax")
    sql_lines.append("-- ============================================")
    sql_lines.append("")
    ...
    for subject, topics in sorted(subject_topics.items()):
        ...
        for topic_name, topic_data in sorted(topics.items()):
            ...
            # FIXED: Use INSERT with VALUES, subquery for subject_official_code
            sql_lines.append(f"")
            ...
            sql_lines.append(f"    (SELECT subject_official_code FROM lookup_subjects WHERE name = '{subject.replace("'", "''")}' LIMIT 1),")
    ...
    sql_file = "caps_atp_seed_fixed.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Fixed SQL saved to: {sql_file}")
