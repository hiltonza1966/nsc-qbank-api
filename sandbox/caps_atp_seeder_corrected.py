#!/usr/bin/env python3
"""
QBank CAPS Database Seeder - CORRECTED for Actual Schema
Uses ALL columns from lookup_caps_topics and lookup_caps_subtopics.
No fields dropped.
"""
...
def generate_seed_sql():
    ...
    sql_lines.append("-- CORRECTED for actual schema - ALL fields used")
    sql_lines.append("-- ============================================")
    sql_lines.append("")
    ...
    for subject, topics in sorted(subject_topics.items()):
        ...
        for topic_name, topic_data in sorted(topics.items()):
            ...
            # Extract grade_number from grades (e.g., "10-12" → 10)
            ...
            # Extract term (use first term if multiple)
            terms_list = sorted(topic_data['terms'])
            term = str(terms_list[0]) if terms_list else 'NULL'
            if term != 'NULL':
                term = f"'{term}'"

            # Generate topic_code from topic name
            topic_code = re.sub(r'[^a-zA-Z0-9]', '_', topic_name[:20]).upper()
            ...
            # Build INSERT for lookup_caps_topics - ALL COLUMNS
            sql_lines.append(f"")
            ...
            sql_lines.append(f"    ls.subject_official_code,")  
            sql_lines.append(f"    NULL,")  # grade_id - will be linked later
            sql_lines.append(f"    {grade_number},")  # grade_number
            sql_lines.append(f"    {strand},")  # strand
            sql_lines.append(f"    {term},")  # term
            sql_lines.append(f"    {topic_code},")  # topic_code
            sql_lines.append(f"    '{topic_clean}',")  # topic_name
            sql_lines.append(f"    NULL,")  # topic_weighting
            sql_lines.append(f"    NULL,")  # time_weeks
            sql_lines.append(f"    NULL,")  # paper_no
            sql_lines.append(f"    '{description}',")  # description
            sql_lines.append(f"    1,")  # is_active
            sql_lines.append(f"    {display_order}")  # display_order
    ...
    # Save SQL
    sql_file = "caps_atp_seed_corrected.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Corrected SQL saved to: {sql_file}")
