#!/usr/bin/env python3
"""
QBank CAPS Database Seeder - FIXED v6
Proper counter logic that checks final truncated code.
"""
...
def generate_seed_sql():
    ...
    sql_lines.append("-- FIXED v6: proper counter on final truncated code")
    sql_lines.append("-- ============================================")
    sql_lines.append("")
    ...
    # Track used topic_codes to ensure uniqueness
    used_topic_codes = set()
    ...
    for subject, topics in sorted(subject_topics.items()):
        ...
        # Generate short subject prefix (3 chars max)
        subject_prefix = re.sub(r'[^a-zA-Z0-9]', '', subject[:3]).upper()
        ...
        for topic_name, topic_data in sorted(topics.items()):
            ...
            # FIXED v6: Generate unique topic_code with proper counter
            topic_abbr = re.sub(r'[^a-zA-Z0-9]', '', topic_name[:10]).upper()
            base_code = f"{subject_prefix}_{topic_abbr}"

            # Start with truncated base (max 17 chars to leave room for _XX)
            final_code = base_code[:17]

            # Check uniqueness and add counter if needed
            counter = 1
            test_code = final_code
            while test_code in used_topic_codes:
                # Try with counter suffix: _01, _02, etc.
                suffix = f"_{counter:02d}"
                test_code = base_code[:17 - len(suffix)] + suffix
                counter += 1
                # Safety check: if we can't fit suffix, truncate more
                if len(test_code) > 20:
                    test_code = base_code[:15] + suffix
                if counter > 99:
                    # Extreme case: use hash
                    test_code = base_code[:14] + f"_{counter:03d}"

            final_code = test_code
            used_topic_codes.add(final_code)
            topic_code = f"'{final_code}'"
            ...
            sql_lines.append(f"    {strand},")
            sql_lines.append(f"    {term},")
            sql_lines.append(f"    {topic_code},")  # FIXED v6: proper unique code
    ...
    sql_file = "caps_atp_seed_v6.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Fixed SQL v6 saved to: {sql_file}")
