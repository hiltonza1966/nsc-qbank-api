#!/usr/bin/env python3
"""
QBank CAPS Database Seeder - FIXED v5
Adds counter to topic_code to ensure uniqueness even after truncation.
"""
...
def generate_seed_sql():
    ...
    sql_lines.append("-- FIXED v5: counter-based unique topic_code")
    sql_lines.append("-- ============================================")
    sql_lines.append("")
    ...
    for subject, topics in sorted(subject_topics.items()):
        ...
        for topic_name, topic_data in sorted(topics.items()):
            ...
            # FIXED v5: Generate unique topic_code with counter
            topic_abbr = re.sub(r'[^a-zA-Z0-9]', '', topic_name[:10]).upper()
            base_code = f"{subject_prefix}_{topic_abbr}"

            # Ensure uniqueness by adding counter if needed
            counter = 1
            topic_code = base_code[:17]  # Leave room for _XX suffix
            while topic_code in used_topic_codes:
                topic_code = f"{base_code[:15]}_{counter:02d}"
                counter += 1

            used_topic_codes.add(topic_code)
            ...
            sql_lines.append(f"    {strand},")
            sql_lines.append(f"    {term},")
            sql_lines.append(f"    {topic_code},")  # FIXED v5: unique with counter
    ...
    sql_file = "caps_atp_seed_v5.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Fixed SQL v5 saved to: {sql_file}")
