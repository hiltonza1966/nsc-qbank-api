#!/usr/bin/env python3
"""
QBank CAPS Database Seeder - FIXED v7
Uses sequential numbers for topic_code to guarantee uniqueness.
"""
...
def generate_seed_sql():
    ...
    sql_lines.append("-- FIXED v7: sequential numbers for topic_code")
    sql_lines.append("-- ============================================")
    sql_lines.append("")
    ...
    topic_count = 0
    subtopic_count = 0
    topic_counter = 1  # Global sequential counter for topic_code
    ...
    for subject, topics in sorted(subject_topics.items()):
        ...
        for topic_name, topic_data in sorted(topics.items()):
            ...
            # FIXED v7: Use sequential number for topic_code (guaranteed unique)
            ...
            sql_lines.append(f"    {strand},")
            sql_lines.append(f"    {term},")
            sql_lines.append(f"    {topic_code},")  # FIXED v7: sequential number
    ...
    sql_file = "caps_atp_seed_v7.sql"
    with open(sql_file, 'w', encoding='utf-8') as f:
        f.write("\n".join(sql_lines))

    print(f"\n✓ Fixed SQL v7 saved to: {sql_file}")
