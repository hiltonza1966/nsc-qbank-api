...
if (!fs.existsSync(filePath)) { console.error('File not found'); process.exit(1); }

let content = fs.readFileSync(filePath, 'utf8');

// FIX 1: Insert QP Gallery before Question Text label
...
if (qtIdx === -1) { console.error('Question Text label not found'); process.exit(1); }
console.log('Found Question Text label at:', qtIdx);
...
let newContent = content.substring(0, qtIdx) + qpGallery + content.substring(qtIdx);
console.log('Inserted QP Image Gallery');

// FIX 2: Replace Attachments Section (16 spaces indentation)
const startMarker = '                {/* Attachments Section */}';
const startIdx = newContent.indexOf(startMarker);
if (startIdx === -1) { console.error('Attachments Section not found. Searching with different spacing...'); 
  // Try with 18 spaces
  const altMarker = '                  {/* Attachments Section */}';
  const altIdx = newContent.indexOf(altMarker);
  if (altIdx === -1) { console.error('Still not found'); process.exit(1); }
  console.log('Found with 18 spaces at:', altIdx);
}
console.log('Found Attachments Section at:', startIdx);

const loadingMarker = "{attachmentLoading && <div style={{ fontSize: '11px', color: '#6b7280' }}>Loading attachments...</div>}";
const loadingIdx = newContent.indexOf(loadingMarker, startIdx);
if (loadingIdx === -1) { console.error('Loading marker not found'); process.exit(1); }

const endDivIdx = newContent.indexOf('</div>', loadingIdx + loadingMarker.length);
if (endDivIdx === -1) { console.error('Closing </div> not found'); process.exit(1); }
const endPos = endDivIdx + '</div>'.length;

const oldSection = newContent.substring(startIdx, endPos);
...
newContent = newContent.substring(0, startIdx) + newSection + newContent.substring(endPos);

const backupPath = filePath + '.bak.v4_' + new Date().toISOString().replace(/[:.]/g, '-');
