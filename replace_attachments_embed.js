...
// =============================================================================
// Find the Question Text textarea section and add image gallery above it

// Look for the pattern: <div style={{ fontSize: '12px', fontWeight: '600', color: '#374151', marginBottom: '4px' }}>Question Text</div>
...
if (qtIdx === -1) {
  console.error('ERROR: Could not find Question Text label');
  process.exit(1);
}
console.log('Found Question Text label at position:', qtIdx);

// Insert QP Image Gallery BEFORE the Question Text label
const qpGallery = `
...
if (startIdx === -1) {
  console.error('ERROR: Could not find Attachments Section start marker');
  process.exit(1);
}
...
if (loadingIdx === -1) {
  console.error('ERROR: Could not find loading marker');
  process.exit(1);
}

const endDivIdx = contentWithGallery.indexOf('</div>', loadingIdx + loadingMarker.length);
if (endDivIdx === -1) {
  console.error('ERROR: Could not find closing </div>');
  process.exit(1);
}
...
console.log('  1. Added QP Image Gallery inline above Question Text (200px thumbnails)');
console.log('  2. Replaced Attachments Section with view-only grouped display (80px thumbnails)');
console.log('  3. Images are clickable to open full size in new tab');
console.log('  4. Delete button only for manually uploaded attachments');
