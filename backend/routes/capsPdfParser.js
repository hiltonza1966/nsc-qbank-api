/**
 * Generic CAPS PDF Parser — routes/capsPdfParser.js (v2.1 HEADER-BASED FIXED)
 * Fixed: Header-based block parsing, real assessment names, weightings, duration
 * Fixed: No longer flattens text - preserves line structure for extraction
 * Added: ATP, Programmes of Assessment, Recording & Reporting parsers
 */
...
// ============================================================================
// HEADER-BASED PARSER CONSTANTS
// ============================================================================

const HEADER_PATTERNS = {
  // Assessment Programme headers - flexible spacing
  term: [
  ...
  skills: /Skills\s*[:\s]/i,
  // Programmes of Assessment headers
...
class CapsPdfParser {
  ...
  _detectDocumentType() {
    const fullText = this.rawText.substring(0, 5000).toLowerCase();

    if (fullText.includes('annual teaching plan') || fullText.includes('atp') || (fullText.includes('week') && fullText.includes('topic'))) {
      this.documentType = 'annual_teaching_plan';
      return;
    }
    if (fullText.includes('programmes of assessment') && fullText.includes('promotion')) {
      this.documentType = 'programmes_of_assessment';
      return;
    }
    if (fullText.includes('recording') && fullText.includes('reporting') && fullText.includes('code of conduct')) {
  ...
  // ============================================================================
  // SECTION 4 EXTRACTION (UNCHANGED)
  // ============================================================================
  ...
  // ============================================================================
  // ASSESSMENT PROGRAMME PARSER (HEADER-BASED + FALLBACK)
  // ============================================================================
  ...
  _extractGradeBlock(text, gradeNum) {
    ...
    // FIXED: More flexible validation for concatenated text
    ...
    if (!hasTerm && !hasTest && !hasExam && !hasAssessment) {
      return null;
    }
  ...
  // ============================================================================
  // HEADER-BASED TERM BLOCK PARSER (FIXED - preserves line structure)
  // ============================================================================

  _parseByHeaders(gradeBlock, gradeNum) {
    const assessments = [];

    // FIXED: Do NOT flatten the text - preserve line structure
    // The original gradeBlock from pdf-parse already has proper newlines
