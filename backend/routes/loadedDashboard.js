...
router.get('/loaded', async (req, res) => {
  try {
    ...
    // Get all papers with item counts, memo counts, attachment counts
...
router.get('/loaded/:paperCode', async (req, res) => {
  try {
    ...
    // Check if item_attachments table exists
    let hasAttachments2 = false;
    try {
      await db.query('SELECT 1 FROM item_attachments LIMIT 1');
      hasAttachments2 = true;
    } catch (e) {
      hasAttachments2 = false;
    }

    const attachmentJoin2 = hasAttachments2 
      ? 'LEFT JOIN item_attachments att ON im.item_id = att.item_id'
      : '';
    const attachmentCount2 = hasAttachments2 
    ...
    const [items] = await db.query(`
      SELECT 
        ...
        im.created_at,
        imem.memo_id,
        imem.answer_text as memo_answer,
        imem.marks as memo_marks,
        imem.marking_guideline,
        ${attachmentCount2}
      FROM item_master im
      LEFT JOIN item_memos imem ON im.item_id = imem.item_id
      ${attachmentJoin2}
