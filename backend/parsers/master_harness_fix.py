...
FIX: Return full item objects in result arrays for frontend review panel.
"""
...
def run_harness(qp_path, memo_path, paper_code):
    ...
    if qp_items:
        print(f"  Sample: {qp_items[0]['question_number']} - {qp_items[0]['marks']} marks - text: {qp_items[0]['question_text'][:50]}")
    ...
    # FIX: Return full item objects in arrays so frontend has all data
    result = {
        ...
        'red_count': len(red),
        # Return FULL item objects, not just q+issue
        'red_items': [{
            'q': r['question_number'],
