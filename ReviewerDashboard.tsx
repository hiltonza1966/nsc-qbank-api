...
import { MessageSquare, CheckCircle, XCircle, AlertCircle, Clock, User, BookOpen, Filter, Send, History } from 'lucide-react';
...
const ReviewerDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'review' | 'history' | 'threads'>('review');
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/v2/review/items-for-review?user_id=${userId}&role=${userRole}&status=${filterStatus}`);
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        <span className="flex items-center gap-1">
                          <span className="font-medium">Marks:</span> {item.marks}/{item.marks_allocated}
                        </span>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-gray-600">Marks Allocated:</span> <span className="font-medium">{selectedItem.marks_allocated}</span></div>
                        <div className="flex justify-between"><span className="text-gray-600">Extracted Marks:</span> <span className="font-medium">{selectedItem.marks}</span></div>
